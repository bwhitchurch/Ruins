...
function probability(percent) return math.random() <= percent end
...
script.on_load(function() spawnTable = global.spawnTable end)
script.on_event(defines.events.on_chunk_generated, function(e)
    local center = {
        x = (e.area.left_top.x + e.area.right_bottom.x) / 2,
        y = (e.area.left_top.y + e.area.right_bottom.y) / 2
    }
    if math.abs(center.x) < settings.global["ruins-min-distance-from-spawn"].value and math.abs(center.y) <
    ...
    if spawnType <= spawnTable.small then
        -- spawn small ruin
        if DEBUG then game.print("A small ruin was spawned at " .. center.x .. "," .. center.y) end
    ...
    elseif spawnType <= spawnTable.medium then
        -- spawn medium ruin
        if DEBUG then game.print("A medium ruin was spawned at " .. center.x .. "," .. center.y) end
    ...
    elseif spawnType <= spawnTable.large then
        -- spawn large ruin
        if DEBUG then game.print("A large ruin was spawned at " .. center.x .. "," .. center.y) end
